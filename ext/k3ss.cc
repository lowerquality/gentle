...
int main(int argc , char *argv[])
{
    ...
    using namespace fst;
  // config
  const  std::string model_dir = "exp/SpanishModel";
  const  std::string graph_dir = model_dir + "graph_pp";
  const  std::string fst_rxfilename = graph_dir + "/HCLG.fst";
    ...
    const std::string model_rxfilename = model_dir + "/final.mdl";
...
//what would be the Adaptation state? in nnet3 we used
  ...
  SingleUtteranceGmmDecoder decoder(sim_decoding_config,
      ...
      while(true) {
        ...
        if(strcmp(cmd,"stop\n") == 0) {
          ...
          // No silence weighting No ivectors
